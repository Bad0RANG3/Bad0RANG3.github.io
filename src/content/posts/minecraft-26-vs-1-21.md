...
从 1.21 直接跳到 26.2，很多旧命令会当场失效。NBT 写法、附魔字段、`item` 和 `data` 的职责都变了，数据包作者如果照着旧教程迁移，通常会在第一条命令上就撞墙。

下面按日常使用频率整理主要变化，尽量把“以前怎么写、现在怎么写、为什么变”放在一起说清楚。命令示例保留原格式，方便直接复制测试。
...
在 1.21 及更早版本里，给物品附加属性靠的是 NBT（Named Binary Tag）。

```text
/give @p netherite_sword{display:{Name:'{"text":"断罪之刃","color":"dark_red"}'},Enchantments:[{id:"minecraft:sharpness",lvl:255}]}
```

大括号一层套一层，名称里面还要再包一段 JSON。写一把带名字和附魔的剑，少一个引号就要从头找。

### 26.2 的组件系统

26.x 把物品属性改成了组件格式：每一类属性有名字，值单独写在后面。结构和 JSON 更接近，可读性也高了一个量级。
...
组件使用命名空间区分来源，名称、描述、附魔各占一项。出错时，报错会直接指到具体组件，而不是丢一句“命令无效”让你自己猜。
...
需要注意的是，生存模式中的铁砧仍然受原版上限约束，255 级只在命令和数据包层面有效。换句话说，这是给地图作者和测试用的口子，不是给生存玩法开的后门。
...
26.2 直接使用 JSON 对象。新增的 `obfuscated` 属性可以制作动态乱码，中文字符也能正常渲染。不过字符闪烁很快，长时间观看并不舒服；需要装饰时，Unicode 符号配合柔和颜色反而更稳定。
...
现在属性归属性系统，物品归物品系统，容器也有对应命令。这个变化会让迁移时的命令拆得更散，但每一步都更容易查出错误。
...
从 1.21 到 26.2，命令系统变化最大的地方在于：数据和物品不再共用一套大 NBT。组件负责物品，属性负责实体数值，Display Entity 和 Macro 则补上了地图展示与模板复用。
